...
@section('title', 'Mutasi Guru Pelajaran')
    ...
    <div class="mb-6 flex items-center">
        ...
        <div>
            <h2 class="text-2xl font-black text-slate-800 tracking-tight flex items-center">
                <i class="fas fa-exchange-alt text-indigo-500 mr-3"></i> Mutasi Pengajar
            </h2>
            <p class="text-sm font-bold text-slate-400 mt-0.5">Ganti guru tanpa menghapus riwayat absen guru sebelumnya.</p>
        </div>
    </div>
    ...
    <form action="/master-jadwal-harian/{{ $jadwal->id }}/mutasi" method="POST" class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="bg-slate-50 border-b border-slate-100 p-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
            <div>
                ...
                <h3 class="text-xl font-black text-slate-700">{{ $jadwal->pelajaran->nama_pelajaran ?? 'Pelajaran' }} - Kelas {{ $jadwal->kelas->nama_kelas ?? '-' }}</h3>
            </div>
            <div class="bg-indigo-50 border border-indigo-100 px-4 py-2 rounded-xl">
                <p class="text-[10px] uppercase font-bold tracking-widest text-indigo-400 mb-0.5">Jadwal</p>
                <p class="text-sm font-black text-indigo-600"><i class="fas fa-clock mr-1"></i> Hari {{ $jadwal->hari }} (Jam ke-{{ $jadwal->jam_ke }})</p>
            </div>
        </div>
        ...
        <div class="p-6 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                ...
                <!-- Ikon Panah Pemisah Tengah (Hanya tampil di desktop) -->
                ...
                <div class="bg-amber-50/50 border border-amber-100 rounded-2xl p-5 relative overflow-hidden">
                    ...
                    <div class="flex items-center mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-base font-black text-slate-700">{{ $jadwal->guru->nama_guru ?? 'Tidak diketahui' }}</p>
                        </div>
                    </div>
                    
                    <p class="text-xs font-medium text-amber-700/80 leading-relaxed bg-white/60 p-3 rounded-xl border border-amber-100">
                        <i class="fas fa-info-circle mr-1"></i> Sistem akan secara otomatis mengakhiri masa tugas pengajar ini pada <strong class="text-amber-800">1 hari sebelum</strong> tanggal efektif yang Anda pilih di bawah.
                    </p>
                </div>
                ...
                <div class="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 relative overflow-hidden">
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-emerald-600/70 uppercase tracking-wider mb-1.5">Tanggal Efektif (Mulai Mengajar)</label>
                        <input type="date" name="tanggal_efektif" required value="{{ date('Y-m-d') }}" class="w-full bg-white border border-emerald-200 text-slate-700 text-sm font-bold rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 block p-3 outline-none transition-all shadow-sm cursor-pointer">
                    </div>
                </div>
        ...
        <div class="bg-slate-50 border-t border-slate-100 p-6 flex items-center justify-end">
            ...
            <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-bold text-sm rounded-xl transition-all shadow-[0_4px_15px_-3px_rgba(79,70,229,0.4)] flex items-center">
                <i class="fas fa-save mr-2"></i> Proses Mutasi Guru
            </button>
        </div>
    </form>
</div>
