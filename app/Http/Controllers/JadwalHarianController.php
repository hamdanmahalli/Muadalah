...
class JadwalHarianController extends Controller
{
    public function __construct(
        protected JadwalConflictService $conflict,
        protected JadwalScheduleService $schedule,
        protected JadwalDragDropService $dragDrop,
    ) {}
    ...
    public function store(Request $request)
    {
        ...
        $periodeAktif = get_periode_aktif();
        if (!$periodeAktif) {
            return redirect()->back()->with('error_popup', 'DITOLAK!<br>Anda belum mengaktifkan Periode di Master Periode.');
        }
        ...
        if ($request->guru_id) {
            $bentroks = $this->conflict->bentrokGuruDiKelasLain(
                $request->guru_id, $request->hari, $arrayJam, $tahunAjaran, $request->kelas_id
            );
        ...
        // ===== KECERDASAN 1.5: CEK BENTROK KELAS (SLOT TERISI GURU LAIN) =====
        $slotTerisi = $this->conflict->slotTerisiKelas($request->kelas_id, $request->hari, $arrayJam, $tahunAjaran);
        ...
        $plot = $this->conflict->plotUntuk($request->kelas_id, $request->pelajaran_id);

        if ($plot) {
            $totalSesiLain = $this->conflict->sesiTerpakaiDiLuar($request->kelas_id, $request->pelajaran_id, $tahunAjaran, $arrayJam);

            if (($totalSesiLain + $jumlahJamDiisi) > $plot->beban_jam) {
                if ($force_swap == 'true' && $swap_target) {
                    [$hariLama, $jamLamaStr] = explode('_', $swap_target);
                    $arrayJamLama = explode('-', $jamLamaStr);

                    JadwalHarian::where('kelas_id', $request->kelas_id)
                    ...
                    // Siapkan opsi tukar
                    $terpakaiLain = JadwalHarian::where('kelas_id', $request->kelas_id)
                        ->where('pelajaran_id', $request->pelajaran_id)
                        ->where('tahun_ajaran', $tahunAjaran)
                    ...
                    foreach ($groupedByHari as $hari => $items) {
                        ...
                        foreach ($items as $item) {
                            if (empty($currentSub)) {
                                ...
                                if ($item->jam_ke == $lastItem->jam_ke + 1) {
                                    $currentSub[] = $item;
                                } else {
                                    $first = $currentSub[0];
                                    $last = end($currentSub);
    ...
    {
        $this->schedule->destroyJadwal($id);
    ...
    public function prosesDragDrop(Request $request)
    {
        ...
        try {
            ...
            if (strpos($targetJamInput, '-') !== false) {
                [$start, $end] = explode('-', $targetJamInput);
                $targetJam = range((int) $start, (int) $end);
            } else {
            ...
            \Illuminate\Support\Facades\Log::error('Proses blok jadwal gagal: ' . $e->getMessage());
    ...
    // ==========================================================
    // FORM & EKSEKUSI MUTASI GURU PER-SLOT (Effective-Dated)
    // ==========================================================
    public function formMutasi($id)
    {
        $jadwal = JadwalHarian::with(['guru', 'pelajaran', 'kelas'])->findOrFail($id);
        $semuaGuru = $this->schedule->daftarGuruPengganti($jadwal->guru_id);
    ...
    public function mutasiGuru(Request $request, $jadwal_id)
    {
        ...
        $jadwalLama = $this->schedule->mutasiGuruPerSlot($jadwal_id, $request->guru_baru_id, $request->tanggal_efektif);
